...
<template>
  <div class="w-full rounded-xl border border-(--border-default) bg-(--surface-panel) p-4 shadow-sm">
    ...
    <div class="mt-3 space-y-3">
      ...
      <div v-if="draft.priority || draft.severity || draft.estimate || (draft.dataSensitivity && draft.dataSensitivity !== 'none')" class="flex flex-wrap gap-1.5">
        <span v-if="draft.priority" class="bg-primary-500/10 text-primary-600 dark:text-primary-400 inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[11px] font-medium">
          <UIcon name="i-lucide-signal" class="size-3" />
          {{ draft.priority }}
        </span>
        <span v-if="draft.severity" class="inline-flex items-center gap-1 rounded-full bg-red-500/10 px-2 py-0.5 text-[11px] font-medium text-red-600 dark:text-red-400">
          <UIcon name="i-lucide-alert-triangle" class="size-3" />
          {{ draft.severity }}
        </span>
        <span v-if="draft.estimate" class="inline-flex items-center gap-1 rounded-full bg-blue-500/10 px-2 py-0.5 text-[11px] font-medium text-blue-600 dark:text-blue-400">
          <UIcon name="i-lucide-clock" class="size-3" />
          {{ draft.estimate }}
        </span>
        <span v-if="draft.multiItem" class="inline-flex items-center gap-1 rounded-full bg-amber-500/10 px-2 py-0.5 text-[11px] font-medium text-amber-600 dark:text-amber-400">
          <UIcon name="i-lucide-layers" class="size-3" />
          Multi-item
        </span>
        <span v-if="draft.dataSensitivity && draft.dataSensitivity !== 'none'" class="inline-flex items-center gap-1 rounded-full bg-amber-500/10 px-2 py-0.5 text-[11px] font-medium text-amber-600 dark:text-amber-400">
